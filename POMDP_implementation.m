...
% we won't consider habit so we won't inser matrix E
...
[A, B, C, D, V] = building_matrices(0.1, 3, 4);  % CHANGE this parameters to simulate risk-aversion.
...
for t = 1:T
    ...
    num_iter = 25; % we're using equations that derive from a gradient descent on F so we need convergence
    ...
    if t < T % We only plan and act if there is still a future left to experience
        ...
        for pi = 1:num_policies
            ...
            G(pi) = compute_policy_G(pi, t, T, qs, A, C, V, H_A, num_obs, num_states);
            % calculate G
            %{
